...
from deepdown_simple_data_loader import load_target_data, rename_dimensions_variables

dataloader_logging = PythonLogger("dataloader")


class rcmMchDataset(DownscalingDataset):
    """Custom reader for RCM - MeteoSwiss data pairs over Switzerland."""
...
def _load_dataset(data_path, group, variables, period, stack_axis=1, tmp_path='./tmp'):
    ...
    # Low resolution coordinates to ensure a 30x52 matrix for the RCM dataset
    lon_min = 4.3
    lon_max = 12.25
    lat_min = 45.4
    lat_max = 48.1
    ...
    if group == 'input':
        ...
        for t_enum, target in enumerate(variables):
            if target == 't':
                data_dir_LR = os.path.join(data_path, 'downscaling', 'cordex', 'cordex-ALPS-allyear', 'train',
                                           'tas_day_EUR-11_ALADIN63_CNRM-CM5_r1i1p1_rcp85_ALPS_cordexgrid_train-period.nc')
            elif target == 'tp':
                data_dir_LR = os.path.join(data_path, 'downscaling', 'cordex', 'cordex-ALPS-allyear', 'train',
                                           'pr_day_EUR-11_ALADIN63_CNRM-CM5_r1i1p1_rcp85_ALPS_cordexgrid_train-period.nc')
            elif target == 't_min':
                data_dir_LR = os.path.join(data_path, 'downscaling', 'cordex', 'cordex-ALPS-allyear', 'train',
                                           'tasmin_day_EUR-11_ALADIN63_CNRM-CM5_r1i1p1_rcp85_ALPS_cordexgrid_train-period.nc')
            elif target == 't_max':
                data_dir_LR = os.path.join(data_path, 'downscaling', 'cordex', 'cordex-ALPS-allyear', 'train',
                                           'tasmax_day_EUR-11_ALADIN63_CNRM-CM5_r1i1p1_rcp85_ALPS_cordexgrid_train-period.nc')

            dataloader_logging.info(f"Loading {data_dir_LR}.")

            data_rcm = xr.open_dataset(data_dir_LR)

            data_rcm = rename_dimensions_variables(data_rcm)

            data_LR = data_rcm[target]
            data_rcm.close()

            start_date = np.datetime64(years[0] + '-01-01T12:00:00.000000000')
            end_date = np.datetime64(years[1] + '-12-31T12:00:00.000000000')

            data_LR = data_LR.sel(time=slice(start_date, end_date))

            data_LR = data_LR.where((data_LR.lat >= lat_min) & (data_LR.lat <= lat_max) &
                                    (data_LR.lon >= lon_min) & (data_LR.lon <= lon_max),
                                    # The selection above on a irregular grid will lead to some values
                                    # being masked as whatever is indicated for 'other' (NaN by default).
                                    other=0.0,
                                    drop=True)
        ...
        for t_enum, target in enumerate(variables):
            ...
            if target == 't':
                data_dir_HR = [os.path.join(data_path, 'DeepDown', 'MeteoSwiss', 'TabsD_v2.0_swiss.lv95')]
            elif target == 'tp':
                data_dir_HR = [os.path.join(data_path, 'DeepDown', 'MeteoSwiss', 'RhiresD_v2.0_swiss.lv95')]
            elif target == 't_min':
                data_dir_HR = [os.path.join(data_path, 'DeepDown', 'MeteoSwiss', 'TminD_v2.0_swiss.lv95')]
            elif target == 't_max':
                data_dir_HR = [os.path.join(data_path, 'DeepDown', 'MeteoSwiss', 'TmaxD_v2.0_swiss.lv95')]
