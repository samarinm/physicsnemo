...
def _load_dataset(data_path, group, variables, period, stack_axis=1, tmp_path='./tmp'):
    ...
    # Low resolution coordinates to ensure a 30x52 matrix for the E-OBS dataset
    ...
    if group == 'input':
        ...
        for t_enum, target in enumerate(variables):
            ...
            if target == 't':
                data_dir_LR = [os.path.join(data_path, 'EOBS', 'temperature')]
            elif target == 'tp':
                data_dir_LR = [os.path.join(data_path, 'EOBS', 'precipitation')]
            elif target == 't_min':
                data_dir_LR = [os.path.join(data_path, 'EOBS', 'min_temperature')]
            elif target == 't_max':
                data_dir_LR = [os.path.join(data_path, 'EOBS', 'max_temperature')]

            # if target == 't':
            #     data_dir_LR = [os.path.join(data_path, 'E-OBS', 'temperature')]
            # elif target == 'tp':
            #     data_dir_LR = [os.path.join(data_path, 'E-OBS', 'precipitation')]
            # elif target == 't_min':
            #     data_dir_LR = [os.path.join(data_path, 'E-OBS', 'min_temperature')]
            # elif target == 't_max':
            #     data_dir_LR = [os.path.join(data_path, 'E-OBS', 'max_temperature')]

            dataloader_logging.info(f"Loading {data_dir_LR}.")

            # Bounding box for selecting shape (30,52) in EOBS
            # (necessary for current implementation of data loader)
